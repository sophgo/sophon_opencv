...
public:
    ...
    virtual Ptr<IVideoCapture> createCapture(const std::string &filename, int id = 0) const = 0;
    virtual Ptr<IVideoWriter>  createWriter(const std::string &filename, int fourcc, double fps, const cv::Size &sz, bool isColor, int id=0, const std::string &encodeParams="") const = 0;
};
...
typedef Ptr<IVideoWriter>  (*FN_createWriter)(const std::string& filename, int fourcc, double fps, const Size& sz, bool isColor, int id, const std::string &encodeParams);
