...
public:
    ...
    Ptr<IVideoCapture> createCapture(const std::string &filename, int id) const CV_OVERRIDE;
    Ptr<IVideoWriter>  createWriter(const std::string &filename, int fourcc, double fps, const cv::Size &sz, bool isColor, int id, const std::string &encodeParams) const CV_OVERRIDE;
};
...
public:
            ...
            const std::string &filename, int camera, int id = 0)
    {
        ...
        if (plugin_api->Capture_open)
        {
            ...
            if (CV_ERROR_OK == plugin_api->Capture_open(filename.empty() ? 0 : filename.c_str(), camera, &capture, id))
            {
                CV_Assert(capture);
...
public:
    ...
    Ptr<PluginWriter> create(const OpenCV_VideoIO_Plugin_API_preview* plugin_api,
            const std::string &filename, int fourcc, double fps, const cv::Size &sz, bool isColor, int id=0, const std::string &encodeParams="" )
    {
        CV_Assert(plugin_api);
        ...
        if (plugin_api->Writer_open)
        {
            ...
            if (CV_ERROR_OK == plugin_api->Writer_open(filename.empty() ? 0 : filename.c_str(), fourcc, fps, sz.width, sz.height, isColor, &writer, id, encodeParams.c_str()))
            {
                CV_Assert(writer);
...
Ptr<IVideoCapture> PluginBackend::createCapture(const std::string &filename, int id) const
{
    try
    {
        if (plugin_api_)
            return PluginCapture::create(plugin_api_, filename, 0, id); //.staticCast<IVideoCapture>();
    }
    catch (...)
    {
...
Ptr<IVideoWriter> PluginBackend::createWriter(const std::string &filename, int fourcc, double fps, const cv::Size &sz, bool isColor, int id=0, const std::string &encodeParams="" ) const
{
    try
    {
        if (plugin_api_)
            return PluginWriter::create(plugin_api_, filename, fourcc, fps, sz, isColor, id, encodeParams); //.staticCast<IVideoWriter>();
    }
    catch (...)
    {
